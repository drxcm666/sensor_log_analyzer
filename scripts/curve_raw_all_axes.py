...
def main() -> int:
    ap = argparse.ArgumentParser(description="Calibration curves (RAW) for X/Y/Z axes.")
    ...
    for axis in ("x", "y", "z"):
        out_path = args.outdir / f"calib_curve_{axis}_raw.png"
        plot_curve(
            points,
            axis=axis,
            key="raw_mean",
            out_path=out_path,
            title=f"Calibration curve (raw) — {axis.upper()} axis",
        )
...
# python scripts\curve_raw_all_axes.py --json .\data\data_calib.json
